...
//
// This fork's whole point: readable text on HiDPI panels while the console
// stays at native GOP resolution. Two mechanisms compose:
//
//  - an embedded high-resolution bitmap font (BigFont.c, generated from a
//    PSF2 console font at build time) drawn directly by this driver, with
//    the platform's HII 8x19 glyphs stretched into the cell as a fallback
//    for characters the font lacks;
//  - integer glyph magnification (GLYPH_SCALE) with nearest-neighbor
//    sampling on top of whichever base font is active.
//
// The effective magnification is chosen per device at Start() and degrades
// toward 1 so the UEFI-mandated 80x25 mode always fits the panel; on panels
// too small for even one 80x25 grid of big-font cells the driver falls back
// to the stock 8x19 HII rendering path entirely.
//
#ifndef GLYPH_SCALE
...
//
// Embedded high-resolution font (BigFont.c, build-generated). Glyph bitmaps
// are 1bpp rows of (mBigFontWidth+7)/8 bytes, mBigFontGlyphSize bytes per
// glyph; the map is sorted by Char for binary search.
//
typedef struct {
  UINT16    Char;
  UINT16    Glyph;
...
extern CONST UINT32              mBigFontWidth;
extern CONST UINT32              mBigFontHeight;
extern CONST UINT32              mBigFontGlyphSize;
extern CONST UINT8               mBigFontGlyphs[];
extern CONST BIG_FONT_MAP_ENTRY  mBigFontMap[];
extern CONST UINTN               mBigFontMapSize;
...
//
// Effective on-screen text cell dimensions for a device, in pixels.
//
#define CELL_WIDTH(Private)   ((Private)->CellWidth)
#define CELL_HEIGHT(Private)  ((Private)->CellHeight)
